...
Operating notes for AI agents (and humans) working with Bouro. Everything here is derivable from
this repository — no private context is required to reach the operational level described.

## What Bouro is, in one paragraph

Bouro is the local-first knowledge graph and epistemic ledger of a three-tool loop: an outbound
execution engine ([Ouro](https://github.com/semigrp/ouro)) runs pinned work, Bouro owns durable
meaning (Concept / Claim / Question / Hypothesis / Evidence / Decision), and
[Fukuro](https://github.com/semigrp/fukuro) owns telemetry and baselines. Bouro never executes
...
npm link                       # puts `bouro` on PATH

# one personal vault, reachable from anywhere (resolution: --vault > $BOURO_VAULT > ./vault/store.json)
export BOURO_VAULT="$HOME/path/of/your/choice/store.json"   # add to your shell profile
bouro init
bouro doctor                   # must print ok: true, errors: []
```
...
```bash
bouro concept --title "operating principles" \
...
- **Ouro**: set `BOURO_BIN=$(which bouro)` so Ouro runs can query context and deliver Evidence.
- **Fukuro**: after registering a unit, `fukuro log-event concept_captured --data '{"bouro_id":"CLM-n"}'`
...
2. **History is append-only; meaning changes by supersession.** When a principle evolves:
   register the new Claim, `bouro relate --type revises --from CLM-new --to CLM-old`, then
   `bouro revise --id CLM-old --status superseded`. Never rewrite an asserted statement into a
   different meaning.
...
```bash
bouro doctor | status | audit --limit 20
bouro claim --title "..." --statement "..." --concept CON-n
bouro question --title "..." --question "..." --closure-rule "..." --concept CON-n
bouro relate --type revises --from CLM-new --to CLM-old
bouro revise --id QST-n --status closed
bouro context --root CON-n --purpose "..."         # version-pinned bundle for downstream use
bouro show --id CLM-n | history --id CLM-n
```

Run `bouro doctor` after any batch of writes; it must stay `ok: true`.
