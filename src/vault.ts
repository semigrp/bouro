...
export function getRevision(store: Store, ref: ResourceRefV1): KnowledgeRevision {
  if (ref.system !== "boros") throw new Error(`Cannot resolve non-Boros ref: ${ref.system}`);
...
export function registerEvidence(
  ...
  if (command.schema !== "boros.register-evidence/v1") {
...
export function createContextBundle(store: Store, query: ContextQueryV1): ContextBundle {
  if (query.schema !== "boros.context-query/v1") throw new Error("Unsupported context query schema");
...
export function validateStore(store: Store): ValidationResult {
  ...
  for (const ref of Object.values(store.heads)) {
    ...
    if (
      ...
      !revision.provenance.derivedFrom.some((item) => item.system === "boros" && item.type === KINDS.decision)
    ) {
...
export function createDemo(store: Store): Record<string, KnowledgeRevision | ContextBundle> {
  ...
  const procedure = createProcedure(store, {
    ...
    purpose: "Verify that an Ouro run can reproduce its Boros knowledge inputs.",
    ...
    implementations: [
      {
        system: "github",
        type: "file",
        id: "semigrp/boros:procedures/replay.ts",
        version: "demo-commit",
...
export function migrateStore(raw: unknown): Store {
  ...
  throw new Error("Unsupported Boros store schema or version");
}
...
function migrateV1(raw: unknown): Store {
  ...
  store.legacy = { migratedFrom: "boros.store/v1", objects: [], edges: [], events: [...events] };
  ...
  for (const rawObject of objects) {
    ...
    if (kind === KINDS.decision) {
      ...
      migratedAttributes.evidence = evidenceIds.map((id) =>
        jsonRef({ system: "boros", type: KINDS.evidence, id, version: "1" }),
      );
    }
...
function collectRevisionErrors(revision: KnowledgeRevision, errors: string[]): void {
  ...
  if (revision.schema !== "boros.knowledge-revision/v1") {
...
function validatePersistentRef(
  ...
  if (ref.system === "boros" && ref.type === "context_bundle") {
  ...
  if (ref.system === "boros" && Object.values(KINDS).includes(ref.type as Kind)) {
    try {
      getRevision(store, ref);
    } catch (error) {
...
function assertIngressRef(store: Store, ref: ResourceRefV1): void {
  ...
  if (ref.system === "boros" && ref.type === "ontology_release") {
  ...
  if (ref.system === "boros") throw new Error(`Unknown Boros resource type: ${ref.type}`);
...
function relationRef(relation: KnowledgeRelation): ResourceRefV1 {
  return { system: "boros", type: "knowledge_relation", id: relation.id, version: "1" };
}
